...
async function gmeplay(url, tracknum, settingsObject /*contains LoopObject (loopStart (milliseconds), loopEnd), length (never used with LoopObject), panningObject*/) {
	...
	*/
	...
	var totalSamples=musLength/* in milliseconds */ * 44100/*sample rate*/ / 1000 // total samples in the song. // note to myself: this can be a float, should something be done about this?
	...
	bufferlength=(musLength/1000)*audioCtx.sampleRate // this should equal totalSamples
	if (panningObject) {
		var MusRecBuffer=[] // contains an array of Audio Context buffers for each voice
		for (let i=0; i<totalVoices; i++) {
			if (MusRec[i]!=null) {
				MusRecBuffer[i]=audioCtx.createBuffer(
					1, // mono
					bufferlength,
					audioCtx.sampleRate
				);
				let nowBuffering=MusRecBuffer[i].getChannelData(0)
				for (let j = 0; j < bufferlength; j++) { // fills the buffer with the data from MusRec
					nowBuffering[j]=MusRec[i][j]
				}
			}
		}
		var MusRecBufferOther=audioCtx.createBuffer(
			1,
			bufferlength,
			audioCtx.sampleRate
		);
		let nowBuffering=MusRecBufferOther.getChannelData(0)
		for (let i = 0; i < bufferlength; i++) {
			nowBuffering[i]=MusRecOther[i]
		}
		sources=[] // an array of AudioBufferSourceNodes for each voice.
		stereoPannerNodes=[] // an array of StereoPannerNodes for each voice.
	} else {
		if (monobool) { /*system is mono*/
			console.log("creating Audio Context buffer for mono")
			var MusRecBuffer=audioCtx.createBuffer(
				1, // mono
				bufferlength,
				audioCtx.sampleRate
			);
			let nowBuffering=MusRecBuffer.getChannelData(0)
			for (let i = 0; i < bufferlength; i++) {
				nowBuffering[i]=MusRec[i]
			}
		} else { // system is stereo
			console.log("creating Audio Context buffer for stereo")
			var MusRecBuffer=audioCtx.createBuffer(
				2, // stereo
				bufferlength,
				audioCtx.sampleRate
			);
			for (let channel=0, numOfChannels=2; channel<numOfChannels; channel++) {
				let nowBuffering=MusRecBuffer.getChannelData(channel)
				for (let i = 0; i < bufferlength; i++) {
					nowBuffering[i]=MusRec[channel][i]
				}
			}
		}
	}
	...
	if (LoopObject) {
		...
		let loopStartval=0;
		// use the user-defined loopStart. if there is no user-defined loopStart, use the intro_length value in the file's metadata.
		if (LoopObject.loopStart) {loopStartval=LoopObject.loopStart} else {loopStartval=Module.ccall("getIntroLength","number")};
		// if the file turns out to have no intro_length metadata, set loop start to 0
		if (loopStartval=== -1) {loopStartval=0}
		sourceNodeSettingsObject={loop:true, loopEnd:musLength/1000, loopStart:loopStartval/1000}
	} else {
		console.log("no LoopObject")
	}
	if (!monobool && !panningObject) { /*system is stereo && panningObject is not defined */
		sourceNodeSettingsObject.channelCount=2
	} else { // system is mono, or panningObject is being used
		sourceNodeSettingsObject.channelCount=1
	}
	console.log(sourceNodeSettingsObject)
	if (panningObject) {
		for (let i=0; i<totalVoices; i++) { // create a source node and set panning for each voice
			if (MusRecBuffer[i]!=null){
				sources[i] = new AudioBufferSourceNode(audioCtx, sourceNodeSettingsObject)
				sources[i].buffer=MusRecBuffer[i]
				...
				} else { // this should never activate, right?
		...
		sourceOther.buffer=MusRecBufferOther
		...
		// this shouldn't have any difference between mono and stereo systems
		var source=new AudioBufferSourceNode(audioCtx, sourceNodeSettingsObject)
		source.buffer=MusRecBuffer
...
function gmeplayWorker(url, tracknum, settingsObject /*contains LoopObject (loopStart (milliseconds), loopEnd), length (never used with LoopObject), panningObject*/) {
	// May error out. Try to only use this function with spc and vgm, which take a long time to play
	
	const audioCtx=new AudioContext({latencyHint:"playback",sampleRate:44100});
	
	const gmeWorker = new Worker("gmeplay-worker.js");
	
	gmeWorker.onmessage = (e) => {
		console.log("Message received from worker");
		var MusRec=e.data[0];
		var musLength=e.data[1]
		var monobool=e.data[2]
		var totalVoices=e.data[3]
		var MusRecOther=e.data[4]
		var VoiceDict=e.data[5]
		gmeWorker.terminate()
		...
		bufferlength=(musLength/1000)*audioCtx.sampleRate
		if (panningObject) {
			var MusRecBuffer=[] // contains an array of Audio Context buffers for each voice
			for (let i=0; i<totalVoices; i++) {
				if (MusRec[i]!=null) {
		...
		if (LoopObject) {
			...
			let loopStartval=0;
			// use the user-defined loopStart. if there is no user-defined loopStart, use the intro_length value in the file's metadata.
			if (LoopObject.loopStart) {loopStartval=LoopObject.loopStart} else {loopStartval=e.data[6]};
			// if the file turns out to have no intro_length metadata, set loop start to 0
			if (loopStartval=== -1) {loopStartval=0}
			sourceNodeSettingsObject={loop:true, loopEnd:musLength/1000, loopStart:loopStartval/1000}
		} else {
	...
	gmeWorker.postMessage({url:url, tracknum:tracknum, audioCtxProperties:{baseLatency:audioCtx.baseLatency, sampleRate:audioCtx.sampleRate}, settingsObject:settingsObject})
}